...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, resolution, num_heads, window_size=8, shift_size=0, mlp_ratio=4., qkv_bias=True, qk_scale=None, act_layer=nn.GELU, norm_layer=nn.BatchNorm2d):
        super().__init__()
        self.dim = dim
...
class BasicLayer(nn.Module):
    def __init__(self, dim, resolution, embed_dim=50, depth=2, num_heads=8, window_size=8, mlp_ratio=1., qkv_bias=True, qk_scale=None, norm_layer=None):
        super().__init__()
        self.dim = dim
        ...
        self.window_size = window_size
        # build blocks
        self.blocks = nn.ModuleList([SwinTransformerBlock(dim=dim, resolution=resolution,
...
class DCTLSA(nn.Module):
    def __init__(self, in_nc=3, nf=55, num_modules=6, out_nc=3, upscale=4,num_head=5):
        ...
        # //The DCTB code Start
        self.B1 = LSAB(in_channels=nf,num_head=num_head)   #125.62K
        self.B2 = LSAB(in_channels=nf,num_head=num_head)   #125.62K
        self.B3 = LSAB(in_channels=nf,num_head=num_head)   #125.62K
        self.B4 = LSAB(in_channels=nf,num_head=num_head)   #125.62K
        self.B5 = LSAB(in_channels=nf,num_head=num_head)   #125.62K
        self.B6 = LSAB(in_channels=nf,num_head=num_head)   #125.62K
        self.c = conv_block(nf * num_modules, nf, kernel_size=1, act_type='lrelu')  #18.2K
        self.c1 = conv_block(nf * 2, nf, kernel_size=1, act_type='lrelu')  #6.11K
        self.c2 = conv_block(nf * 3, nf, kernel_size=1, act_type='lrelu')  #9.13K
        self.c3 = conv_block(nf * 4, nf, kernel_size=1, act_type='lrelu')  #12.15K
        self.c4 = conv_block(nf * 5, nf, kernel_size=1, act_type='lrelu')  #15.18K
        self.c5 = conv_block(nf * 6, nf, kernel_size=1, act_type='lrelu')  #18.2K
        # //The DCTB code End
        self.LR_conv = conv_layer(nf, nf, kernel_size=3)  #27.28
        upsample_block = pixelshuffle_block
        self.upsampler = upsample_block(nf, out_nc, upscale_factor=upscale)  #23.81
        self.scale_idx = 0
